...
func parseLayout[T core.Numeric](dim int, input *core.Tensor[T]) (layout, error) {
	...
	if input == nil || len(input.Data) == 0 {
		return lay, fmt.Errorf("rmsnorm: empty input")
	}
	...
	case 2:
		...
		if input.Shape[1] != dim {
			return lay, fmt.Errorf("rmsnorm: width %d != Dim %d", input.Shape[1], dim)
		}
	case 3:
		lay.batch = input.Shape[0]
		lay.seq = input.Shape[1]
		if input.Shape[2] != dim {
			return lay, fmt.Errorf("rmsnorm: last dim %d != Dim %d", input.Shape[2], dim)
		}
	default:
		return lay, fmt.Errorf("rmsnorm: shape need [batch,dim] or [batch,seq,dim], got %v", input.Shape)
	}
	if lay.batch <= 0 || lay.seq <= 0 {
		return lay, fmt.Errorf("rmsnorm: invalid batch/seq")
	}
	want := lay.batch * lay.seq * dim
	if len(input.Data) < want {
		return lay, fmt.Errorf("rmsnorm: data len %d < %d", len(input.Data), want)
	}
	return lay, nil
}
