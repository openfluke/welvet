...
// Layer is Conv3d. Weights live on Proj (Dense Filters × InChannels·K³).
type Layer struct {
	Core core.Layer
	Cfg  Config
...
// New creates CNN3 with zero FormatNone weights.
func New(cfg Config) (*Layer, error) {
	return NewConfigured[float64](cfg, core.DTypeFloat32, quant.FormatNone, nil)
}

// NewConfigured builds CNN3 from flat init [filters × in × k × k × k] (loom / PyTorch).
func NewConfigured[T core.Numeric](cfg Config, dt core.DType, format quant.Format, init []T) (*Layer, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("cnn3 proj: %w", err)
	}
	l := &Layer{
		Core: core.Layer{
			Type:         core.LayerCNN3,
...
func Forward[T core.Numeric](l *Layer, input *core.Tensor[T]) (pre, post *core.Tensor[T], err error) {
	if l == nil || l.Proj == nil || input == nil {
		return nil, nil, fmt.Errorf("cnn3: nil layer/input")
	}
	l.syncProjExec()
...
func Backward[T core.Numeric](l *Layer, gradOut, input, pre *core.Tensor[T]) (gradIn, gradW *core.Tensor[T], err error) {
	if l == nil || l.Proj == nil {
		return nil, nil, fmt.Errorf("cnn3: nil layer")
	}
	l.syncProjExec()
	switch l.Exec.Backend {
	case core.BackendSIMD:
...
// AllPermutations lists the CNN3 coverage matrix.
