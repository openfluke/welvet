...
func parseLayout[T core.Numeric](cfg Config, input *core.Tensor[T]) (layout, error) {
	...
	if input == nil || len(input.Data) == 0 {
		return lay, fmt.Errorf("rnn: empty input")
	}
	if len(input.Shape) != 3 {
		return lay, fmt.Errorf("rnn: shape need [batch,seq,input], got %v", input.Shape)
	}
	lay.batch = input.Shape[0]
	if input.Shape[1] != lay.seq {
		return lay, fmt.Errorf("rnn: seq %d != %d", input.Shape[1], lay.seq)
	}
	if input.Shape[2] != lay.in {
		return lay, fmt.Errorf("rnn: input dim %d != %d", input.Shape[2], lay.in)
	}
	if lay.batch <= 0 {
		return lay, fmt.Errorf("rnn: invalid batch")
	}
	want := lay.batch * lay.seq * lay.in
	if len(input.Data) < want {
		return lay, fmt.Errorf("rnn: data len %d < %d", len(input.Data), want)
	}
	return lay, nil
}
