...
func parseLayout[T core.Numeric](inDim int, input *core.Tensor[T]) (layout, error) {
	...
	if input == nil || len(input.Data) == 0 {
		return lay, fmt.Errorf("residual: empty input")
	}
	...
	case 2:
		...
		if input.Shape[1] != inDim {
			return lay, fmt.Errorf("residual: width %d != Dim %d", input.Shape[1], inDim)
		}
	case 3:
		lay.batch = input.Shape[0]
		lay.seq = input.Shape[1]
		if input.Shape[2] != inDim {
			return lay, fmt.Errorf("residual: last dim %d != Dim %d", input.Shape[2], inDim)
		}
	default:
		return lay, fmt.Errorf("residual: shape need [batch,in] or [batch,seq,in], got %v", input.Shape)
	}
	if lay.batch <= 0 || lay.seq <= 0 {
		return lay, fmt.Errorf("residual: invalid batch/seq")
	}
	lay.elemStride = lay.seq * lay.in
	want := lay.batch * lay.elemStride
	if len(input.Data) < want {
		return lay, fmt.Errorf("residual: data len %d < %d", len(input.Data), want)
	}
	return lay, nil
}
