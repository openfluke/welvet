...
func forwardViaDense[T core.Numeric](l *Layer, input *core.Tensor[T]) (pre, post *core.Tensor[T], err error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("cnn2 dense fwd: %w", err)
	}
	pre = loomFromDense(preFlat.Data, lay.batch, lay.filters, lay.outH, lay.outW)
	post = loomFromDense(postFlat.Data, lay.batch, lay.filters, lay.outH, lay.outW)
	return pre, post, nil
}
...
func backwardViaDense[T core.Numeric](l *Layer, gradOut, input, pre *core.Tensor[T]) (gradIn, gradW *core.Tensor[T], err error) {
	...
	if gradOut == nil || pre == nil {
		return nil, nil, fmt.Errorf("cnn2: nil gradOut/pre")
	}
	wantOut := lay.batch * lay.filters * lay.outH * lay.outW
	if gradOut.Len() < wantOut || pre.Len() < wantOut {
		return nil, nil, fmt.Errorf("cnn2: grad/pre short")
	}
	col := im2col(input, lay)
	gy := denseFromLoom(gradOut.Data, lay.batch, lay.filters, lay.outH, lay.outW)
	preFlat := denseFromLoom(pre.Data, lay.batch, lay.filters, lay.outH, lay.outW)
	gxCol, dW, err := dense.Backward(l.Proj, gy, col, preFlat)
	if err != nil {
		return nil, nil, fmt.Errorf("cnn2 dense bwd: %w", err)
	}
