...
// Layer is ConvTranspose1d. Weights live on Proj (Dense Filters × InChannels·Kernel).
type Layer struct {
	Core core.Layer
	Cfg  Config
...
// New creates ConvT1 with zero FormatNone weights.
func New(cfg Config) (*Layer, error) {
	return NewConfigured[float64](cfg, core.DTypeFloat32, quant.FormatNone, nil)
}

// NewConfigured builds ConvT1 from flat init [filters × in × k] (loom / PyTorch layout).
func NewConfigured[T core.Numeric](cfg Config, dt core.DType, format quant.Format, init []T) (*Layer, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("convt1 proj: %w", err)
	}
	l := &Layer{
		Core: core.Layer{
			Type:         core.LayerConvTransposed1D,
			DType:        dt,
...
func Forward[T core.Numeric](l *Layer, input *core.Tensor[T]) (pre, post *core.Tensor[T], err error) {
	if l == nil || l.Proj == nil || input == nil {
		return nil, nil, fmt.Errorf("convt1: nil layer/input")
	}
	l.syncProjExec()
...
func Backward[T core.Numeric](l *Layer, gradOut, input, pre *core.Tensor[T]) (gradIn, gradW *core.Tensor[T], err error) {
	if l == nil || l.Proj == nil {
		return nil, nil, fmt.Errorf("convt1: nil layer")
	}
	l.syncProjExec()
	switch l.Exec.Backend {
	case core.BackendSIMD:
...
// AllPermutations lists the ConvT1 coverage matrix.
