...
// ForwardSIMD requires Plan 9 SIMD, then runs the host decode loop (projections via quant.MatVec).
func ForwardSIMD[T core.Numeric](l *Layer, input *core.Tensor[T]) (pre, post *core.Tensor[T], err error) {
	if !simd.Enabled() {
		return nil, nil, fmt.Errorf("gdn: BackendSIMD but Plan 9 SIMD not enabled")
	}
	return forwardHost(l, input)
}

// BackwardSIMD requires Plan 9 SIMD, then runs truncated-BPTT host backward.
