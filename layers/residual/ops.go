...
// NewFromOps builds Residual F over mixed Ops (Dense, SwiGLU, RMSNorm, LayerNorm).
// Parallel as F is parallel.ResidualGraft (import cycle).
func NewFromOps(cfg Config, ops []any) (*Layer, error) {
	if err := cfg.Validate(); err != nil {
		return nil, err
	}
	if len(ops) == 0 {
		return nil, fmt.Errorf("residual: NewFromOps needs ≥1 child")
	}
	for i, op := range ops {
		if op == nil {
			return nil, fmt.Errorf("residual: nil F child at %d", i)
		}
		if !supportedChild(op) {
			return nil, fmt.Errorf("residual: unsupported F %T at %d (no Parallel — use ResidualGraft)", op, i)
		}
	}
	cfg.Depth = len(ops)
	l := &Layer{
...
func callFwd[T core.Numeric](op any, input *core.Tensor[T]) (pre, post *core.Tensor[T], err error) {
	...
	default:
		return nil, nil, fmt.Errorf("residual: fwd unsupported %T", op)
	}
}
...
func callBwd[T core.Numeric](op any, gradOut, input, pre *core.Tensor[T]) (gradIn, gradW *core.Tensor[T], err error) {
	...
	default:
		return nil, nil, fmt.Errorf("residual: bwd unsupported %T", op)
	}
}
...
func childApplySGD[T core.Numeric](op any, dW *core.Tensor[T], lr float64) error {
	...
	default:
		return fmt.Errorf("residual: SGD unsupported %T", op)
	}
}
...
func childSetDType(op any, dt core.DType) error {
	...
	case *layernorm.Layer:
		return v.SetDType(dt)
	default:
		return fmt.Errorf("residual: SetDType unsupported %T", op)
	}
}
...
func childPack(op any, format quant.Format) error {
	...
	case *layernorm.Layer:
		return v.Pack(format)
	default:
		return fmt.Errorf("residual: Pack unsupported %T", op)
	}
}
