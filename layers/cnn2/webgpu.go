...
func forwardTiledWebGPU[T core.Numeric](l *Layer, input *core.Tensor[T]) (pre, post *core.Tensor[T], err error) {
	...
	nIn := lay.batch * lay.inC * lay.inH * lay.inW
	...
	if err != nil {
		return nil, nil, fmt.Errorf("cnn2 tiled fwd weights: %w", err)
	}
	cfg := webgpu.CNN2Config{
		Batch: lay.batch, InC: lay.inC, InH: lay.inH, InW: lay.inW,
		OutC: lay.filters, OutH: lay.outH, OutW: lay.outW,
		KSize: lay.kSize, Stride: lay.stride, Padding: lay.padding,
		MultiCore: l.Exec.MultiCore || l.Core.MultiCore,
	}
	if err := webgpu.CNN2Forward(xF, wF, preF, cfg); err != nil {
		return nil, nil, fmt.Errorf("cnn2 tiled fwd: %w", err)
	}
...
func backwardTiledWebGPU[T core.Numeric](l *Layer, gradOut, input, pre *core.Tensor[T]) (gradIn, gradW *core.Tensor[T], err error) {
	...
	if gradOut == nil || pre == nil || gradOut.Len() < wantOut || pre.Len() < wantOut {
		return nil, nil, fmt.Errorf("cnn2: grad/pre shape")
	}

	gradIn = core.NewTensor[T](lay.batch, lay.inC, lay.inH, lay.inW)
	patch := lay.inC * lay.kSize * lay.kSize
	...
	wF, err := l.Proj.Weights.GPUWireF32()
	if err != nil {
		return nil, nil, fmt.Errorf("cnn2 tiled bwd weights: %w", err)
	}
	cfg := webgpu.CNN2Config{
		Batch: lay.batch, InC: lay.inC, InH: lay.inH, InW: lay.inW,
		OutC: lay.filters, OutH: lay.outH, OutW: lay.outW,
	...
	if err := webgpu.CNN2Backward(gyF, wF, inF, preF, gxF, gwF, cfg, l.Cfg.Activation); err != nil {
		return nil, nil, fmt.Errorf("cnn2 tiled bwd: %w", err)
	}
